...
const fmt = (n) => (n / crore).toFixed(1);

const WORK_TYPE_COLORS = {
  'Housing': '#FF9933',
  'Livelihood': '#4ade80',
  'Road Construction': '#60a5fa',
  'Sanitation': '#f472b6',
  'Water Supply': '#34d399',
};
...
const Funds = () => {
    ...
    fetch('/data/up_large_dataset.csv')
      ...
      .then(text => {
        ...
        const rows = lines.slice(1).map(l => {
          const [state, district, village, scheme, allocated_fund, used_fund, year, work_type] = l.split(',');
          return { scheme, allocated: parseInt(allocated_fund), used: parseInt(used_fund), work_type, year };
        });

        // Scheme aggregates
        const schemeMap = {};
        rows.forEach(r => {
          if (!schemeMap[r.scheme]) schemeMap[r.scheme] = { allocated: 0, used: 0, latestYear: '' };
          schemeMap[r.scheme].allocated += r.allocated;
          schemeMap[r.scheme].used += r.used;
          if (r.year > schemeMap[r.scheme].latestYear) schemeMap[r.scheme].latestYear = r.year;
        });

        const schemeList = Object.entries(schemeMap).map(([name, v], i) => ({
          id: `SCM-${301 + i}`,
          name,
          budget: v.allocated,
          used: v.used,
          remaining: v.allocated - v.used,
          progress: Math.round((v.used / v.allocated) * 100),
          lastYear: v.latestYear,
        }));
        ...
        const wtMap = {};
        rows.forEach(r => {
          if (!wtMap[r.work_type]) wtMap[r.work_type] = 0;
          wtMap[r.work_type] += r.allocated;
        });
        const total = Object.values(wtMap).reduce((a, b) => a + b, 0);
        setTotalAllocated(total);
        setTotalUsed(rows.reduce((a, r) => a + r.used, 0));
        setWorkTypes(Object.entries(wtMap).map(([name, val]) => ({
          name,
  ...
  const topAlert = schemes.find(s => s.progress >= 50);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <div className="px-6 py-2.5 bg-green-50 border border-green-100 text-[#138808] rounded-2xl shadow-sm flex items-center gap-3">
            ...
            <span className="text-xs font-black uppercase tracking-widest">₹{fmt(totalUsed)} Cr Disbursed</span>
          </div>
          <button
      ...
      {showForm && (
        <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-blue-50 animate-in zoom-in-95 duration-300 relative overflow-hidden group mb-10">
          ...
          <div className="flex items-center gap-4 relative z-10 pt-4">
            <button type="button" className="px-10 py-3.5 bg-blue-900 text-white rounded-2xl shadow-xl shadow-blue-100 hover:bg-blue-950 transition-all font-black uppercase tracking-widest text-xs flex items-center gap-3 active:scale-[0.98]">
              <CheckCircle2 className="w-4 h-4" />
              Initialize Project
            </button>
            <button type="button" onClick={() => setShowForm(false)} className="px-6 py-3.5 bg-white border border-slate-200 text-slate-500 rounded-2xl hover:bg-slate-50 transition-all font-black uppercase tracking-widest text-xs">
              Cancel Draft
            </button>
          </div>
        </div>
        ...
        <div className="col-span-1 lg:col-span-2 space-y-8">
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 group relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <div key={i} className="p-6 bg-slate-50/50 rounded-3xl animate-pulse h-24" />
                ))
              ) : (
                schemes.map((scheme) => (
                  <div key={scheme.id} className="p-6 bg-slate-50/50 rounded-3xl group/item hover:bg-blue-50/50 transition-all border-2 border-transparent hover:border-blue-100/50">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            ...
                            Latest Year: {scheme.lastYear}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-sm font-black text-slate-900">₹{fmt(scheme.used)}Cr</span>
                        <span className="text-[10px] font-bold text-slate-400 tracking-tighter uppercase block">Expended of ₹{fmt(scheme.budget)}Cr</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                      ...
                      <span className="text-slate-400 flex items-center gap-1">Remaining: <span className="text-slate-700">₹{fmt(scheme.remaining)}Cr</span></span>
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="space-y-8">
          <div className="bg-[#1E3A8A] p-10 rounded-[3.5rem] shadow-2xl shadow-blue-100 text-white relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-12">
              <div>
                <h3 className="text-sm font-black text-blue-200 uppercase tracking-[0.2em] mb-3">Total Fund Allotment</h3>
                <div className="flex items-baseline gap-3">
                  <h2 className="text-5xl font-black tracking-tighter">₹{fmt(totalAllocated)}</h2>
                ...
                <p className="text-[10px] font-bold text-blue-300/60 uppercase tracking-widest mt-2 flex items-center gap-2">
                  ...
                  Across {schemes.length} Active Schemes
                </p>
              </div>
                ...
                {workTypes.map(wt => (
                  <div key={wt.name}>
                    ...
                    <div className="h-1.5 bg-blue-900/50 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${wt.pct}%`, backgroundColor: WORK_TYPE_COLORS[wt.name] || '#60a5fa' }} />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col items-center justify-center text-center group">
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 px-4 italic leading-relaxed">
              {topAlert
                ? `${topAlert.name} has consumed ${topAlert.progress}% of its budget. Plan realignment recommended.`
                : 'All schemes are within safe utilization limits.'}
            </p>
          </div>
        </div>
      </div>
